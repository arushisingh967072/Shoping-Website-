  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      <div>
        ...
        <div className='flex gap-2'>
          ...
          <label htmlFor="image1">
            ...
            <input onChange={(ev) => setImage1(ev.target.files[0])} type="file" id="image1" hidden />
          </label>

          <label htmlFor="image2">
            <img className="w-20" src={!image2 ? assets.upload_area : URL.createObjectURL(image2)} alt="" />
            <input onChange={(ev) => setImage2(ev.target.files[0])} type="file" id="image2" hidden />
          </label>

          <label htmlFor="image3">
            <img className="w-20" src={!image3 ? assets.upload_area : URL.createObjectURL(image3)} alt="" />
            <input onChange={(ev) => setImage3(ev.target.files[0])} type="file" id="image3" hidden />
          </label>

          <label htmlFor="image4">
            <img className="w-20" src={!image4 ? assets.upload_area : URL.createObjectURL(image4)} alt="" />
            <input onChange={(ev) => setImage4(ev.target.files[0])} type="file" id="image4" hidden />
          </label>
      ...
      <div className='w-full'>
        ...
        <input onChange={(e) => setDescription(e.target.value)} value={description} type="text" placeholder='Enter Name' />
      </div>
      ...
      <button type='submit' className='w-28 py-3 mt-4 bg-pink-200 text-black'>ADD ITEM</button>
